...
from gc_garch.common.copulas import (  # noqa: E402
    fit_static_pair_t_copula,
    simulate_static_pair_t_innovations,
)
...
def write_residual_correlation_csv(path: Path, rows: list[dict[str, Any]]) -> None:
    ...
    fields = ["file", "innovation_corr", "innovation_abs_corr"]
...
def main() -> None:
    ...
    parser.add_argument(
        "--config",
        default="gc_garch/models/pair_t_static/config.json",
    ...
    pair_fit = fit_static_pair_t_copula(marginal_fit["standardized_residuals"], config)

    rng = np.random.default_rng(seed)
    correlation_rows: list[dict[str, Any]] = []
    for idx in range(1, num_generated + 1):
        innovations = simulate_static_pair_t_innovations(
    ...
    write_residual_correlation_csv(
        dirs["data"] / "generated_standardized_residual_correlations.csv",
        correlation_rows,
    )

    payload = {
        "model": "pair_t_static",
        "description": "Two-series static Student-t pair-copula with per-series GJR-GARCH(1,1) marginals.",
    ...
    log = "\n".join(
        [
            ...
            f"burn_in: {burn_in}",
            f"pair_copula_rho: {pair_fit['rho']:.6f}",
            f"pair_copula_df: {pair_fit['degrees_of_freedom']:.6f}",
